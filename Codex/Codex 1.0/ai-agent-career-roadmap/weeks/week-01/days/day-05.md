...
模块是一个 Python 文件，包是包含多个模块的目录。配置应从环境变量或配置对象进入，不要散落在函数中。今天把前几天代码整理为 `src/` 包，让导入关系和入口清楚。
...
- **文件路径：** `weeks/week-01/exercises/day-05/`
...
- **创建文件：** 在 `weeks/week-01/exercises/day-05/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
