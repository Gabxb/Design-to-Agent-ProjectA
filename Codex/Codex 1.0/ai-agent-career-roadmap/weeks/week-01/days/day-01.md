# Day 1：安装 Python 3.12 与建立虚拟环境

## 今日目标

- 建立清晰心智模型，并跑通一个最小例子。
- 产出一个可以打开、运行或检查的文件，而不只停留在阅读。
- 用自己的话记录它与“可运行的需求数据清洗命令行工具”的关系。

## 岗位价值

能独立阅读和修改 Agent 项目的 Python 代码，并建立可复现环境。
...
虚拟环境是项目专属的 Python 工具箱，用来隔离依赖版本。先确认 `python --version` 为 3.12，再用 `python -m venv .venv` 创建环境。激活后安装的包只属于当前项目。把它理解为设计文件内嵌字体与组件版本：缺少隔离时，另一个项目的升级可能让当前项目突然失效。
...
- **文件路径：** `weeks/week-01/exercises/day-01/`
...
- **创建文件：** 在 `weeks/week-01/exercises/day-01/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“虚拟环境”实现一个最小函数、API、检索步骤或工作流节点。
