...
函数把输入、处理和输出封装为可测试单元。异常用于表示无法正常完成的情况，例如需求文本为空。今天要区分“返回空结果”和“抛出错误”：前者是合法业务结果，后者是调用方需要处理的失败。
...
- **文件路径：** `weeks/week-01/exercises/day-03/`
...
- **创建文件：** 在 `weeks/week-01/exercises/day-03/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“函数与模块”实现一个最小函数、API、检索步骤或工作流节点。
