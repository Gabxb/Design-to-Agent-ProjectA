...
独立挑战要求你只根据输入输出和验收标准完成需求清洗器。先写三个示例，再实现代码。遇到错误时保留最小失败输入，阅读 Traceback（错误追踪）最底部的异常类型和信息。
...
- **文件路径：** `weeks/week-01/exercises/day-06/`
...
- **创建文件：** 在 `weeks/week-01/exercises/day-06/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
