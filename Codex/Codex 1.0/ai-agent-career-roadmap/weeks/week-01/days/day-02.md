...
类型提示不是为了让 Python 变成强类型语言，而是提前说明数据契约。`list[str]` 表示字符串列表，`dict[str, int]` 表示键为字符串、值为整数的字典。对 Agent 工程而言，清晰类型能减少工具参数、状态字段和 API 数据之间的误解。
...
- **文件路径：** `weeks/week-01/exercises/day-02/`
...
- **创建文件：** 在 `weeks/week-01/exercises/day-02/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“类型提示”实现一个最小函数、API、检索步骤或工作流节点。
