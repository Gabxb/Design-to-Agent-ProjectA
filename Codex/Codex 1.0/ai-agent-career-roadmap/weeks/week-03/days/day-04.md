...
索引像内容目录，能加快特定查询但增加写入和存储成本。事务让一组操作要么全部成功，要么全部失败。今天用一个唯一约束和一个事务例子理解二者。
...
- **文件路径：** `weeks/week-03/exercises/day-04/`
...
- **创建文件：** 在 `weeks/week-03/exercises/day-04/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
