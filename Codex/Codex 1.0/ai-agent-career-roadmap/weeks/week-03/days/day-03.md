...
SQL（Structured Query Language，结构化查询语言）用于读写关系数据。每次写入都要明确必填字段和默认值；更新与删除必须有条件，避免影响整张表。
...
- **文件路径：** `weeks/week-03/exercises/day-03/`
...
- **创建文件：** 在 `weeks/week-03/exercises/day-03/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“CRUD”实现一个最小函数、API、检索步骤或工作流节点。
