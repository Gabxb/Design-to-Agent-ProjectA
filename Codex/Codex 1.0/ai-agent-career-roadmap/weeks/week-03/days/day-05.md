...
API 层不应直接堆满 SQL。将数据库访问放入单独模块或 Repository（仓储）层，让路由处理 HTTP、服务层处理业务规则、数据层处理持久化。
...
- **文件路径：** `weeks/week-03/exercises/day-05/`
...
- **创建文件：** 在 `weeks/week-03/exercises/day-05/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“数据库连接配置”实现一个最小函数、API、检索步骤或工作流节点。
