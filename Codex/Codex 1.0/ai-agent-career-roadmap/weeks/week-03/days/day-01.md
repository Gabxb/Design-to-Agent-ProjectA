...
关系数据库把稳定业务对象放入表中，用主键唯一标识记录，用外键表达关系。先从用户故事提取实体和字段，避免为了未来可能性建立过多表。
...
- **文件路径：** `weeks/week-03/exercises/day-01/`
...
- **创建文件：** 在 `weeks/week-03/exercises/day-01/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“表与主键”实现一个最小函数、API、检索步骤或工作流节点。
