...
服务层负责业务规则，例如标题标准化、风险等级计算和重复需求判断。路由只负责把请求转换为服务调用，再转换为响应。
...
- **文件路径：** `weeks/week-04/exercises/day-03/`
...
- **创建文件：** 在 `weeks/week-04/exercises/day-03/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“错误响应”实现一个最小函数、API、检索步骤或工作流节点。
