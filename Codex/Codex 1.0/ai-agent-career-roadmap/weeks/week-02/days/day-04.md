...
FastAPI 会根据类型提示和 Pydantic 数据模型自动校验请求并生成接口文档。今天先实现 `/health` 和一个 `POST /requirements`，观察合法与非法请求的差异。
...
- **文件路径：** `weeks/week-02/exercises/day-04/`
...
- **创建文件：** 在 `weeks/week-02/exercises/day-04/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-01-tool-calling/`。
- **实现内容：** 围绕“FastAPI 路由”实现一个最小函数、API、检索步骤或工作流节点。
...
- **执行提示：** 先写一个失败测试，再实现最少代码让它通过。 今天给出了更具体的实现边界，但核心函数仍需独立完成。
