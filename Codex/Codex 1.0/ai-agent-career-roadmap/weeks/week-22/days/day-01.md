# Day 1：Python、API 与数据库面试：概念与最小示例

## 今日目标

- 建立清晰心智模型，并跑通一个最小例子。
- 产出一个可以打开、运行或检查的文件，而不只停留在阅读。
- 用自己的话记录它与“基础面试答案库与两次模拟”的关系。
...
今天处于“概念与最小示例”节奏，核心是围绕 **高频基础题、代码阅读、调试题、API 设计与 SQL** 完成可验证产出。先从本周必须掌握的 Python 数据模型、HTTP、SQL、测试 中选取与项目里程碑最相关的一项，运行最小示例，再接入项目。首次遇到英文缩写时，在学习日志中写出英文全称、中文含义和一个项目例子。

本文件提供完整任务结构和摘要，不替代最新框架文档。实现前使用本周知识库文档中的官方文档名称或检索关键词核对当前 API。建立清晰心智模型，并跑通一个最小例子，并把结果与“基础面试答案库与两次模拟”直接连接。
...
- **文件路径：** `weeks/week-22/exercises/day-01/`
...
- **创建文件：** 在 `weeks/week-22/exercises/day-01/` 创建 `README.md`、一个 Python 练习文件和必要测试；项目改动放在 `projects/project-04-capstone/`。
- **实现内容：** 围绕“Python 数据模型”实现一个最小函数、API、检索步骤或工作流节点。
