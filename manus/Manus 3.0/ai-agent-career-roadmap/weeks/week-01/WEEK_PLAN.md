# Week 01：Python 环境、命令行与可维护脚本

<!-- AUTO_BOOK_NAV_START -->
## 教程书籍导航

- 所属阶段：[阶段 1：软件工程基础](../../overview/OUTLINE.md)
...
- 项目：[设计需求结构化助手 API](../../projects/project-01-tool-calling/README.md)
...
本周处于“编程与本地开发基础”阶段。完成后，你应能说明并实践：虚拟环境、函数、类型提示、文件路径；同时交付 可重复运行的 Python 命令行小工具。
...
| 日程 | 主题 | 建议投入 |
|---|---|---|
| Day 1 | 建立 Python 3.12 虚拟环境与项目骨架 | 2–3 小时 |
| Day 2 | 函数、类型提示与数据验证 | 2–3 小时 |
| Day 3 | 文件读写、JSON 与异常边界 | 2–3 小时 |
| Day 4 | 模块拆分、日志与配置 | 2–3 小时 |
| Day 5 | 命令行最小产品 | 2–3 小时 |
| Day 6 | 独立实现：需求摘要格式化 | 2 小时 |
| Day 7 | 周复习与 Git 提交 | 2 小时 |
...
项目：**设计需求结构化助手 API**。本周里程碑：**可重复运行的 Python 命令行小工具**。将实际实现状态记录到项目 `TASKS.md`。
...
阅读 Week 02 的 `WEEK_PLAN.md`，检查本地运行环境、未完成任务与需准备的非敏感示例数据。
