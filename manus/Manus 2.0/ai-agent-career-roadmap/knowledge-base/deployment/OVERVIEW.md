...
部署将应用、配置、依赖、监控和回滚策略一起交付到可运行环境。

## 为什么 Agent 开发需要它

Agent 系统必须把不确定的模型能力放进确定的工程边界中。Deployment 能帮助你定义输入、保存证据、控制副作用或验证输出，从而让系统可维护、可测试并能交接。

## 设计师可以如何理解

将它看作从原型到真实服务的交付规范，而不仅是把代码放到服务器。
...
如何解释 Deployment 在教学原型与生产服务中的差异？当输入异常、外部依赖超时或模型输出不满足约束时，你的系统如何处理？

## 与其他技术的关系

Deployment 与 API、数据库、测试、部署和安全并非独立知识点；它应当通过清晰契约连接到项目中的数据流、控制流和可观测性设计。

## 官方文档名称或检索关键词

[Docker Documentation](https://docs.docker.com/)。检索关键词：`Deployment Python production guide`。
